...
表格使用 `NAME STATE PATH` 三列。`STATE` 默认只显示当前状态，存在漂移时仅在行首显示 `!`；用户暂存修改后才显示 `当前 → 目标`，并高亮目标状态。行首的 `~`、`×`、`✓` 分别表示待应用、冲突和本次会话已应用；不可管理的 Skill 标为 `只读`。列宽优先保证状态和 Skill 名称，剩余空间才显示路径；窄终端会先隐藏路径。
