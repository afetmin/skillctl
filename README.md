...
表格使用 `NAME STATE DESCRIPTION` 三列。`STATE` 默认只显示当前状态，存在漂移时仅在行首显示 `!`；用户暂存修改后才显示 `当前 → 目标`，并高亮目标状态。行首的 `~`、`×`、`✓` 分别表示待应用、冲突和本次会话已应用；不可管理的 Skill 标为 `只读`。描述保持单行，空值显示 `No description`；完整路径和描述可在详情页查看。搜索同时匹配名称、描述、ID、来源和路径。
